...
// La Kasbah By Beldi (16/09), pack imprimeur (15/09), réponses de Mehdi du 16/09. Aucun montant. Heure du Maroc (UTC+1 en continu).
...
const people = P.map(([nom, prenom, genre, passeport, naissance, regime, ch2, t2, chK, tK, jeep]) => {
  ...
  return {
    ...
    note_hotel: `2Ciels ch. ${ch2} (${t2}, lits séparés) · Agafay tente ⚠️ non communiquée · Kasbah ch. ${chK} (${tK})`,
    note_app: notes.join(' '),
    ...
    a_confirmer: [...(jeep === 1 ? ['Jeep 1 déduite (non listée par Mehdi)'] : []), 'tente Agafay', 'véhicule 4×4 du 20/09'],
  };
});
...
const ALLERGIE = 'ALLERGIE Wim Wildenbeest (noix / arachides, EpiPen) : briefer le chef à l\'arrivée, carte allergie remise, aucune trace. Régimes : Erwin (pas de poisson), Michaël (poisson, beurre, champignons), Marcel (fruits de mer, viande crue), Mischa (coriandre, champignons), Hjalmar (fromage), Roel (ni poisson ni viande), Otte et Adriana (sans gluten).';
const NOTE_HEURE = 'HEURE : le Maroc reste en UTC+1 (heure du téléphone). La fiche transport V3 annonce à tort un passage à GMT : ne PAS reculer les montres.';
const seqs = [
  // J-1
  S('s0916-pack', '2026-09-16', '18:00', '19:30', 'Contrôle du pack imprimeur + brief équipe', 'MRCO', '4', 'setup', 'important', '30 PDF livrés le 15/09. Vérifier les tirages et répartir par lieu (voir Plus → Signalétique & matériel). Cartes allergie : 8 ex., à reconfirmer avec Pamela (liste régimes finale) AVANT impression.', ['Menus pliés par lieu (7 × 22)', 'Chevalets par lieu (7 × 3)', 'Prisme bar A ×2 + totem bar B ×2', 'Sous-verres 2 × 150', 'Pancarte aéroport ×2 + pancartes véhicules ×10 plastifiées', 'Cartes allergie ×8 (après reconfirmation Pamela)', 'Brief Farid / Mouad : rôles J1']),
  S('s0916-najib', '2026-09-16', '19:30', '20:00', 'Appel Najib : flotte confirmée + CORRECTION heure', 'Téléphone', '', 'orga', 'critique', 'Confirmer la disponibilité : 2 × 17 pl (17, 18, 21/09), 4 × 4×4 self drive (19/09), 5 × 4×4 + véhicule bagages (20/09). ' + NOTE_HEURE + ' Sans cette correction, les chauffeurs se présentent 1 h en retard les 20 et 21/09 (vol 09:55).', ['Flotte confirmée sur les 5 jours', 'Correction heure transmise et comprise (pas de changement le 20/09)', 'Numéro de Ssi Brahim obtenu', 'Type du véhicule bagages J4 confirmé']),
  // J1 — jeudi 17/09
  S('s0917-rak', '2026-09-17', '12:30', '13:35', 'MRCO en place à l\'aéroport (2 pers.) + pancarte Sightline', 'Aéroport RAK — zone arrivées', '19', 'setup', 'critique', 'Chauffeurs 2 × 17 pl en place à 13:00. Pancarte « Sightline » (pack 05, A3). Suivi du vol FR5895 en temps réel dans Transport. Sortie passagers +30 à 40 min.', ['2 minibus en place (pancartes BUS 1 / BUS 2)', 'Pancarte aéroport en main', 'Eau fraîche à bord', 'Vol suivi (ETA)']),
  S('s0917-beldi', '2026-09-17', '14:45', '16:30', 'Déjeuner 3 plats — Beldi Country Club (jardins)', 'Beldi Country Club', '19', 'restauration', 'important', 'Thé à la menthe et pâtisseries à l\'arrivée (~14:30). Boissons réglées par le groupe. Menu 2 + chevalets 2. ' + ALLERGIE, ['Chef briefé allergie + régimes', 'Chevalets et menus en place', 'Bagages gardés dans les bus'], ALL),
  S('s0917-checkin', '2026-09-17', '16:45', '17:45', 'Check-in 2Ciels — 12 chambres, lits séparés', '2Ciels Boutique Hotel & Spa', '19', 'orga', 'critique', 'Rooming V5 du 16/09 : 12 chambres maintenues, ch. 2 et 3 en single (Erwin, Arjen), ch. 12 attribuée sur place par le groupe. Bagages déchargés à l\'hôtel. Briefer petit-déjeuner et room service sur l\'allergie de Wim (viennoiseries, fruits secs, amlou, huiles). Temps libre rooftop / spa ensuite.', ['12 clés remises', 'Ch. 12 : occupant noté', 'Bagages montés (19)', 'Allergie briefée (petit-déj + room service)', 'Heure du départ dîner 20:30 annoncée au groupe'], ALL),
  S('s0917-setup-psc', '2026-09-17', '19:30', '20:30', 'Setup MRCO — Petanque Social Club', 'Petanque Social Club', '', 'setup', 'important', 'Menus 1 + chevalets 1 sur les tables, prisme bar A « 30 Years » + totem bar B « Access All Areas », sous-verres Backstage / Proost. Cocktail « 30th Anniversary » à 20:45.', ['Menus et chevalets posés', 'Prisme + totem au bar', 'Sous-verres distribués', 'Chef briefé allergie + régimes']),
  ...
  S('s0918-bazar', '2026-09-18', '12:30', '14:15', 'Déjeuner 3 plats — Le Grand Bazar (Jemaa el-Fna)', 'Le Grand Bazar (Médina)', '19', 'restauration', 'important', 'Dépose au plus près de la médina. Menu 3 + chevalets 3. ' + ALLERGIE, ['Chef briefé allergie + régimes', 'Menus et chevalets en place'], ALL),
  ...
  S('s0918-setup-darna', '2026-09-18', '19:00', '20:00', 'Setup MRCO — Le Comptoir Darna', 'Le Comptoir Darna', '', 'setup', 'important', 'Menus 4 + chevalets 4, sous-verres. Dîner 4 plats avec spectacle (musiciens, danseurs).', ['Menus et chevalets posés', 'Chef briefé allergie + régimes']),
  S('s0918-darna', '2026-09-18', '20:30', '23:30', 'Dîner 4 plats + spectacle — Le Comptoir Darna', 'Le Comptoir Darna', '19', 'restauration', 'important', 'Menu 4. Retour hôtel 23:30, puis option Babouchka (spectacle et entrées offerts par Uyuni) — véhicules en disposal jusqu\'à la dernière course. ' + ALLERGIE, ['Option Babouchka : qui y va ? (compter)', 'Dernière course annoncée aux chauffeurs'], ALL),
  // J3 — samedi 19/09
  ...
  S('s0919-relais', '2026-09-19', '13:00', '14:30', 'Déjeuner 3 plats — Le Relais du Lac (Lalla Takerkoust)', 'Le Relais du Lac', '19', 'restauration', 'important', 'Terrasse au bord du lac, vue Atlas. Menu 5 + chevalets 5. Véhicules en standby. ' + ALLERGIE, ['Chef briefé allergie + régimes', 'Menus et chevalets en place'], ALL),
  ...
  S('s0919-desert', '2026-09-19', '20:00', '00:30', 'Dîner du désert + soirée (tamtam, cracheur de feu, DJ)', 'Yes We Camp Agafay — piscine privatisée', '19', 'restauration', 'important', 'Menu 6, 4 plats. Open bar 3 h inclus (vin, bière, whisky, gin, vodka, softs ; cocktails et spiritueux deluxe en supplément réglé par le groupe). ' + ALLERGIE, ['Chef briefé allergie + régimes', 'Fin de l\'open bar annoncée (3 h)'], ALL),
  S('s0919-heure', '2026-09-19', '22:00', '22:15', 'RAPPEL HEURE : aucun changement cette nuit', 'Yes We Camp Agafay', '', 'orga', 'critique', NOTE_HEURE + ' Vérifier que les téléphones sont en réglage automatique. Confirmer par message à Najib et Ssi Brahim : chauffeurs à 10:30 / 11:00 en heure du téléphone.', ['Message envoyé à Najib', 'Message envoyé à Ssi Brahim', 'Téléphones MRCO en heure automatique']),
  // J4 — dimanche 20/09
  ...
  S('s0920-dej', '2026-09-20', '13:30', '15:00', 'Déjeuner berbère en montagne — chez Adil (Sidi Fares)', 'Imlil / Sidi Fares — setup berbère', '19', 'restauration', 'important', 'Softs inclus. Pas de menu imprimé pour ce déjeuner. ' + ALLERGIE, ['Cuisinier briefé allergie + régimes'], ALL),
...
const waves = [
  ...
  prog('p0917-2030', '2026-09-17', '20:30', '2Ciels → Petanque Social Club', 'Cocktail 20:45, dîner 21:15.', '19', BUS, 'important', [], '2Ciels Boutique Hotel & Spa', 'Petanque Social Club'),
  ...
  prog('p0918-2000', '2026-09-18', '20:00', '2Ciels → Le Comptoir Darna', 'Dîner spectacle 20:30.', '19', BUS, 'important', [], '2Ciels Boutique Hotel & Spa', 'Le Comptoir Darna'),
  ...
  prog('p0920-1030', '2026-09-20', '10:30', 'Véhicule bagages : Agafay → La Kasbah By Beldi', '1 encadrant MRCO à bord (⚠️ à désigner). Type de véhicule à confirmer par Najib. Bagages des 19 + MRCO.', 'bagages', '1 véhicule LUGGAGE', 'critique', [], 'Yes We Camp Agafay', 'La Kasbah By Beldi'),
  prog('p0920-1100', '2026-09-20', '11:00', 'Convoi 5 × 4×4 avec chauffeurs : Agafay → Tahannaout / Asni → Imlil – Sidi Fares → La Kasbah By Beldi (~17:30)', 'Max 4 pax par véhicule (4/4/4/4/3), SANS bagages, + 1 véhicule MRCO. Circuit différent du J3. Répartition nominative ⚠️ à établir. Heure : UTC+1, pas de changement.', '19', '5 × 4×4 (CAR 1-5) + véhicule MRCO', 'critique', ALL, 'Yes We Camp Agafay', 'La Kasbah By Beldi'),
  ...
  W('w0921-0620', '2026-09-21', '06:20', 'depart', 'HV6534 — décollage 09:55 (Eindhoven)', ALL, BUS, 'Chauffeurs en place 05:45, bagages 06:00, départ groupé 06:20, trajet ~45–50 min, dépose ~07:10 (H-2:45). Transavia → Eindhoven 15:35. HEURE : UTC+1, pas de changement.', 'La Kasbah By Beldi', 'critique'),
];
...
const transport = {
  ...
  regles: [
    'HEURE : le Maroc reste en UTC+1 pendant toute l\'opération (heure du téléphone). La fiche transport V3 annonce à tort un passage à GMT dans la nuit du 19 au 20 : NE PAS reculer les montres. Vérification croisée avec Eva / Mehdi avant chaque prise en charge des 20 et 21/09.',
  ...
  contactDispatch: { nom: 'Najib', role: 'Transporteur', tel: '' },
  vagues: waves,
  horsDispatch: [
  ...
  flotte: [
    ...
    { date: '2026-09-18', pax: '19', mouvements: 'Grand Bazar 12:00, standby médina, retour 16:15, Comptoir Darna 20:00, retour 23:30 + disposal', flotte: '2 × 17 pl — 11:30 → fin de soirée' },
...
const lieux = { version: VERSION, updatedAt: UPDATED, diners: [], items: [
  ...
  { id: '2ciels', nom: '2Ciels Boutique Hotel & Spa', adresse: 'Rue Oued El Makhazine, angle rue Imam Chafai, Marrakech', tel: '+212524359550', acces: 'Nuits du 17 au 19/09 (2 nuits). Rooftop, piscine, spa.', quoi: ['12 chambres Luxe, lits séparés : ch. 1, 4-10 twins ; ch. 2, 3, 11, 12 singles (ch. 12 attribuée sur place)', 'Allergie Wim Wildenbeest : petit-déjeuner et room service à briefer', 'Check-in 17/09 ~16:45 · check-out 19/09 08:00, bagages hors chambres 08:30 (transporteur 2Ciels → Agafay)', 'Départ montgolfière 18/09 05:45 (opérateur)'], jours: ['2026-09-17', '2026-09-18', '2026-09-19'] },
  { id: 'psc', nom: 'Petanque Social Club', adresse: 'Marrakech — adresse ⚠️ à confirmer', acces: 'Club secret style années 1930.', quoi: ['17/09 20:45 cocktail « 30th Anniversary »', '17/09 21:15 dîner 4 plats — menu 1, chevalets 1, prisme bar A, totem bar B, sous-verres'], jours: ['2026-09-17'], aConfirmer: true },
  ...
  { id: 'kasbah', nom: 'La Kasbah By Beldi', adresse: 'Route de Lalla Takerkoust — adresse exacte ⚠️ à confirmer', acces: 'Redescente directe depuis Sidi Fares (~17:30). Vers RAK ~45–50 min.', quoi: ['Nuit du 20 au 21/09 : 13 chambres (8 twins + 4 singles + 1 double staff MRCO)', '20/09 20:30 dîner à Kasbah Talet (sur le domaine) — 18 couverts + 2 staff à part — menu 7, chevalets 7', '21/09 petit-déjeuner 05:15, départ 06:20'], jours: ['2026-09-20', '2026-09-21'], aConfirmer: true },
] };

// ---------- Menus (contenu des 7 menus PDF à transcrire — pack imprimeur) ----------
const menus = { version: VERSION, updatedAt: UPDATED, items: [
  { id: 'm1', date: '2026-09-17', sequence: 'Déjeuner J1', lieu: 'Beldi Country Club', menu: 'Menu 2 du pack imprimeur — 3 plats, en cours de transcription ⚠️', boissons: 'Boissons réglées directement par le groupe', aConfirmer: true },
  { id: 'm2', date: '2026-09-17', sequence: 'Dîner J1', lieu: 'Petanque Social Club', menu: 'Menu 1 du pack imprimeur — 4 plats, en cours de transcription ⚠️', boissons: 'Cocktail « 30th Anniversary » 20:45 · boissons réglées par le groupe', aConfirmer: true },
  { id: 'm3', date: '2026-09-18', sequence: 'Déjeuner J2', lieu: 'Le Grand Bazar', menu: 'Menu 3 du pack imprimeur — 3 plats, en cours de transcription ⚠️', boissons: 'Boissons réglées par le groupe', aConfirmer: true },
  { id: 'm4', date: '2026-09-18', sequence: 'Dîner J2', lieu: 'Le Comptoir Darna', menu: 'Menu 4 du pack imprimeur — 4 plats + spectacle, en cours de transcription ⚠️', boissons: 'Boissons réglées par le groupe', aConfirmer: true },
  { id: 'm5', date: '2026-09-19', sequence: 'Déjeuner J3', lieu: 'Le Relais du Lac', menu: 'Menu 5 du pack imprimeur — 3 plats, en cours de transcription ⚠️', boissons: 'Boissons réglées par le groupe', aConfirmer: true },
  { id: 'm6', date: '2026-09-19', sequence: 'Dîner J3', lieu: 'Yes We Camp Agafay', menu: 'Menu 6 du pack imprimeur — 4 plats, en cours de transcription ⚠️', boissons: 'Forfait boissons + open bar 3 h (vin, bière, whisky, gin, vodka, softs) ; cocktails et spiritueux deluxe en supplément', aConfirmer: true },
  { id: 'm7', date: '2026-09-20', sequence: 'Déjeuner J4', lieu: 'Imlil / Sidi Fares — chez Adil', menu: 'Déjeuner berbère (pas de menu imprimé)', boissons: 'Softs inclus' },
  { id: 'm8', date: '2026-09-20', sequence: 'Dîner J4', lieu: 'Kasbah Talet by Beldi', menu: 'Menu 7 du pack imprimeur — 3 plats + pâtisseries marocaines, en cours de transcription ⚠️ · 18 couverts (Wim ne dîne pas) + 2 staff à part', boissons: 'Boissons réglées par le groupe', aConfirmer: true },
] };
...
const contacts = { version: VERSION, updatedAt: UPDATED, groupes: [
  ...
  { nom: 'Transport', contacts: [
    { nom: 'Najib', role: 'Transporteur — 2 × 17 pl, 4×4, véhicule bagages', tel: '', email: '', aConfirmer: true, note: 'Numéro à renseigner' },
  ...
  { nom: 'Lieux', contacts: [
    ...
    { nom: 'Petanque Social Club', role: 'Dîner 17/09', tel: '', email: '', aConfirmer: true },
    ...
    { nom: 'La Kasbah By Beldi / Kasbah Talet', role: 'Nuit 20→21/09 + dîner', tel: '', email: '', aConfirmer: true },
  ] },
] };
...
const signaletique = { version: VERSION, updatedAt: UPDATED, note: 'Pack imprimeur SIGHTLINE 30 (30 PDF, 15/09) : palette orange #EB5E12 / noir / blanc cassé. Quantités du lisez-moi imprimeur.', items: [
  ...
  { id: 'sig-allergie', label: 'Cartes allergie cuisine (A6)', qte: 8, lieu: 'Une par restaurant', date: '2026-09-16', note: 'Reconfirmer la liste régimes avec Pamela AVANT impression', aConfirmer: true },
] };
...
const team = { version: VERSION, updatedAt: UPDATED, membres: ['Mehdi', 'Eva', 'Farid', 'Mouad'],
  affectations: [
    ...
    { sequenceId: 's0917-rak', owner: 'Eva', role: 'accueil aéroport (MRCO 2 pax)' },
...
const pending = { version: VERSION, updatedAt: UPDATED, items: [
  { id: 'pd01', sujet: 'HEURE : la fiche transport V3 annonce un passage à GMT dans la nuit du 19 au 20/09 — FAUX (le Maroc reste en UTC+1, GMT seulement pendant le Ramadan). Corriger auprès de Najib et Ssi Brahim AVANT samedi, sinon chauffeurs 1 h en retard les 20 et 21/09 (vol 09:55)', responsable: 'Mehdi → Najib', echeance: '2026-09-16', level: 'alerte' },
  { id: 'pd02', sujet: 'Jeep 1 (J3) déduite par différence : Tammo van Brakel, Arjen Hofma, Pascal Rintjema, Marcel Vellekoop, Otte Stoltenborg — à confirmer', responsable: 'Mehdi', echeance: '2026-09-18', level: 'alerte', persons: ['van-brakel', 'hofma', 'rintjema-pascal', 'vellekoop', 'stoltenborg'] },
  { id: 'pd03', sujet: 'J4 (20/09) : répartition nominative 5 véhicules de 4/4/4/4/3 à établir et transmettre à Najib / Ssi Brahim (les jeeps du J3 sont à 5)', responsable: 'Mehdi / Eva', echeance: '2026-09-19', level: 'alerte' },
  { id: 'pd04', sujet: 'Véhicule bagages J4 : type et capacité à confirmer par Najib (bagages 19 pax + MRCO) ; encadrant MRCO à bord à désigner', responsable: 'Farid → Najib', echeance: '2026-09-18', level: 'alerte' },
  { id: 'pd05', sujet: 'Convoi J3 : véhicule d\'assistance et position de l\'encadrement MRCO à caler avec Ssi Brahim ; numéro de Ssi Brahim et de Najib à renseigner dans Contacts', responsable: 'Mehdi', echeance: '2026-09-18', level: 'alerte' },
  { id: 'pd06', sujet: 'Cartes allergie (8 ex.) : faire reconfirmer la liste des régimes par Pamela avant impression ; Wim mange-t-il aux autres repas (il ne dîne pas à la Kasbah, repas personnel) ?', responsable: 'Eva → Pamela', echeance: '2026-09-16', level: 'alerte', persons: ['wildenbeest'] },
  ...
  { id: 'pd08', sujet: 'Attribution des 12 tentes Top VIP à Yes We Camp Agafay non communiquée', responsable: 'Eva', echeance: '2026-09-18', level: 'normal' },
  { id: 'pd09', sujet: 'Horaires des soirées J1 et J2 à réconfirmer à J-1 à Najib (fiche V3)', responsable: 'Eva', echeance: '2026-09-16', level: 'normal' },
  { id: 'pd10', sujet: 'Adresses et téléphones manquants : Petanque Social Club, Comptoir Darna, Relais du Lac, Yes We Camp, La Kasbah By Beldi', responsable: 'Eva', echeance: '2026-09-17', level: 'normal' },
  { id: 'pd11', sujet: 'Menus 1 à 7 : transcrire le contenu des PDF du pack dans l\'app (Plus → Menus)', responsable: 'Mehdi (app)', echeance: '2026-09-17', level: 'normal' },
] };
